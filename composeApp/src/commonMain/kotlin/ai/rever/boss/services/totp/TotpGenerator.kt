 ...
 *
 * In `commonMain` (not `desktopMain`) so the commonMain autofill surface
 * (`BrowserSecretIntegrationViewModel`) can call it directly; `javax.crypto` is
 * already used from commonMain elsewhere (`services/importer/browser/ChromiumCrypto`).
 *
 ...
 * published RFC 6238 Appendix B test vectors for all three hash algorithms with no
 * mocking. The seed never leaves this process, the derived key is zeroed after use,
